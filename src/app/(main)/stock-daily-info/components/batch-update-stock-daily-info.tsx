...
import { BatchUpdateStockDailyInfo } from '@/types/stock-daily-info';
import { FormInstance } from 'antd/es/form';
import React, { useMemo } from 'react';


interface BatchUpdateStockDailyInfosProps {
  isBatchUpdateStockDailyInfosModalVisible: boolean;
  onBatchUpdateStockDailyInfosCancel: () => void;
  onBatchUpdateStockDailyInfosFinish: () => void;
  isBatchUpdateStockDailyInfosLoading: boolean;
  batchUpdateStockDailyInfosForm: FormInstance<BatchUpdateStockDailyInfo>;
}

const batchUpdateStockDailyInfosFormItemLayout = {
  labelCol: { span: 6 },
  wrapperCol: { span: 18 },
};

const BatchUpdateStockDailyInfosComponent: React.FC<BatchUpdateStockDailyInfosProps> = ({
  isBatchUpdateStockDailyInfosModalVisible,
  onBatchUpdateStockDailyInfosCancel,
  onBatchUpdateStockDailyInfosFinish,
  isBatchUpdateStockDailyInfosLoading,
  batchUpdateStockDailyInfosForm,
}) => {
  
  const footerButtons = useMemo(
    () => [
      <Button key="cancel" onClick={onBatchUpdateStockDailyInfosCancel}>
        取消
      </Button>,
      <Button key="submit" type="primary" loading={isBatchUpdateStockDailyInfosLoading} onClick={onBatchUpdateStockDailyInfosFinish}>
        确定
      </Button>,
    ],
    [isBatchUpdateStockDailyInfosLoading, onBatchUpdateStockDailyInfosCancel],
  );

  return (
    <Modal
      title="股票每日信息批量编辑"
      open={isBatchUpdateStockDailyInfosModalVisible}
      onCancel={onBatchUpdateStockDailyInfosCancel}
      footer={footerButtons}
      destroyOnHidden
      width={"60%"}
    >
        <Form
          {...batchUpdateStockDailyInfosFormItemLayout}
          form={ batchUpdateStockDailyInfosForm}
          name="batchUpdateStockDailyInfos"
          onFinish={onBatchUpdateStockDailyInfosFinish}
...
export default BatchUpdateStockDailyInfosComponent;